...
class gSpan(object):
    ...
    def __init__(self,
                 ...
                 verbose=False,
                 visualize=False,
                 where=False, check_featureblack = False,graph_dict={},min_support_dict = {}, is_hetero= False):
        ...
        self.vevlb_counter = collections.Counter()
        ...
        self.min_support_dict = min_support_dict
    ...
    def _read_graphs(self, is_ceb=True):
        ...
        if is_ceb:
            ...
            vertexes["node_label_change"] = 1 - vertexes["node_label_0"]
            ...
            ori_graphs = vertexes["group_id"].drop_duplicates()
            ...
            for i in tqdm(range(len(ori_graphs))):
                g_id = ori_graphs.iloc[i]
                ...
                # vertexes_pd["is_origin"] = 0
                # vertexes_pd.loc[vertexes_pd.vertex_id == origin_id, "node is_origin"] = 1
                tgraph.add_vertex_pd(vertexes_pd)
                edges = edges[edges.edge_src != edges.edge_dst]
            ...
            for v in g.vertices.values():
                ...
                for to, e in v.edges.items():
                    ...
                    if self._is_undirected and vlb1 > vlb2:
                        vlb1, vlb2 = vlb2, vlb1
                    if (g.gid, (vlb1, e.elb, vlb2)) not in self.vevlb_counter:
                        self.vevlb_counter[(vlb1, e.elb, vlb2)] += 1
            ...
            for vid, v in g.vertices.items():
                if vid == g.origin_id:
                    ...
                    for e in edges:
                        edge_cnt += 1
                        # 在这里先做一次边的支持度的过滤
                        if self.vevlb_counter[(v.vlb, e.elb, g.vertices[e.to].vlb)] > self._min_support:
                            root[(v.vlb, e.elb, g.vertices[e.to].vlb)].append(
                                PDFS(gid, e, None)
                            )
        print("黑节点数量：",vid_cnt,"边的数量",edge_cnt)

        print("所有黑节点的一度邻居数量：",root.keys(),len(root.values ()))
